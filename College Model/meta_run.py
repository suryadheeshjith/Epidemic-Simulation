...
import numpy as np
from repeated_random_graph.generate_files import write_agents,write_interactions
...
if __name__=="__main__":
    ...
    # Type 1
    fp = open(osp.join(example_path,"results.txt"),"a")
    p = 0.003
    fp.write("Run Type : {0}\n".format(sys.argv[2]))
    for i in range(5):
        # write_agents('repeated_random_graph/agents.txt',1000)
        write_interactions('repeated_random_graph/interactions_list.txt',1000,p)
        world_obj=World.World(example_path, config_obj,model,policy_list,event_restriction_fn,agents_filename,interactions_files_list,locations_filename,events_files_list)
        tdict,_,_,_,_ = world_obj.simulate_worlds(plot=False)
        for key in tdict.keys():
            fp.write("p = {0} : {1} : {2}\n".format(p,key,tdict[key]))
        fp.write("\n")
    fp.write("\n\n\n")
    fp.close()

    # Type 2
    # fp = open(osp.join(example_path,"results2.txt"),"a")
    # fp.write("Run Type : {0}\n".format(sys.argv[2]))
    # p = [0.002,0.0025,0.003,0.0035]
    # for i in range(len(p)):
    #     write_agents('repeated_random_graph/agents.txt',1000)
    #     write_interactions('repeated_random_graph/interactions_list.txt',1000,p[i])
    #     world_obj=World.World(example_path, config_obj,model,policy_list,event_restriction_fn,agents_filename,interactions_files_list,locations_filename,events_files_list)
    #     tdict,_,_,_,_ = world_obj.simulate_worlds(plot=False)
    #     for key in tdict.keys():
    #         fp.write("p = {0} : {1} : {2}\n".format(p[i],key,tdict[key]))
    #     fp.write("\n")
    # fp.write("\n\n\n")
    # fp.close()
